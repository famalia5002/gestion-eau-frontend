...
import { authService, alerteService, avisService, factureService } from '../services/api'
import {
  MdRefresh, MdLightMode, MdDarkMode,
  MdPerson, MdLogout, MdEdit,
  MdNotifications, MdWarning, MdStar,
  MdReceipt, MdError, MdCameraAlt
} from 'react-icons/md'
...
export default function Header({ titre }) {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      const alertesEnCours = alertesRes.data.filter(a => a.statut === 'en_cours')
      alertesEnCours.forEach(a => {
        notifs.push({
          id: `alerte-${a.id}`,
          type: 'alerte',
          titre: a.type_alerte === 'fuite'
      ...
      const avisEnAttente = avisRes.data.filter(a => a.statut === 'en_attente')
      ...
      const facturesEnRetard = facturesRes.data.filter(f => f.statut === 'en_retard')
